...
static func _draw_rat_frame(size: int, bob: float, scurry: float) -> Texture2D:
	...
	_fill_circle(image, size / 2.0, size / 2.0 + 10 * s, 6 * s, Color(0, 0, 0, 0.1))

	# Tail
	_fill_rect(image, cx + 6 * s, cy + 4 * s, 8 * s, 1.5 * s, dark)
	_fill_rect(image, cx + 12 * s, cy + 3 * s + scurry * 0.3, 4 * s, 1.5 * s, dark)

	# Body
	_fill_circle(image, cx, cy + 2 * s, 8 * s, body)
	_fill_circle(image, cx - 2 * s, cy + 4 * s, 5 * s, belly)
	_fill_circle(image, cx + 3 * s, cy + 1 * s, 2 * s, sore)
	_fill_circle(image, cx - 4 * s, cy + 3 * s, 1.5 * s, sore)

	# Legs
	_fill_rect(image, cx - 5 * s, cy + 7 * s + scurry * 0.5, 2 * s, 4 * s, dark)
	_fill_rect(image, cx + 3 * s, cy + 7 * s - scurry * 0.5, 2 * s, 4 * s, dark)

	# Head
	_fill_circle(image, cx - 6 * s, cy - 2 * s, 5 * s, body)
	_fill_circle(image, cx - 10 * s, cy - 2 * s, 3 * s, dark)
	_fill_circle(image, cx - 7 * s, cy - 4 * s, 1.5 * s, Color(0.9, 0.2, 0.1))
	_fill_circle(image, cx - 4 * s, cy - 6 * s, 2.5 * s, body.lightened(0.2))
	_fill_circle(image, cx - 2 * s, cy - 6 * s, 2.5 * s, body.lightened(0.2))

	return ImageTexture.create_from_image(image)

# ─── mushroom shaman sprite frames ───
...
static func _draw_shaman_frame(size: int, bob: float, waddle: float) -> Texture2D:
	...
	_fill_circle(image, size / 2.0, size / 2.0 + 12 * s, 7 * s, Color(0, 0, 0, 0.1))

	# Legs
	_fill_rect(image, cx - 4 * s, cy + 6 * s, 2.5 * s, 6 * s, dark)
	_fill_rect(image, cx + 1.5 * s, cy + 6 * s, 2.5 * s, 6 * s, dark)

	# Body
	_fill_circle(image, cx, cy + 2 * s, 7 * s, stem)
	_fill_circle(image, cx, cy + 4 * s, 5 * s, stem.darkened(0.1))

	# Staff
	_fill_rect(image, cx + 8 * s, cy - 12 * s, 2 * s, 18 * s, Color(0.4, 0.3, 0.2))
	_fill_circle(image, cx + 9 * s, cy - 13 * s, 3 * s, Color(0.3, 0.8, 0.2, 0.8))
	_fill_circle(image, cx + 9 * s, cy - 13 * s, 1.5 * s, Color(0.5, 1.0, 0.3, 0.6))

	# Mushroom cap
	_fill_circle(image, cx, cy - 8 * s, 10 * s, cap)
	_fill_circle(image, cx, cy - 10 * s, 8 * s, cap.lightened(0.15))
	_fill_circle(image, cx - 4 * s, cy - 12 * s, 2 * s, spots)
	_fill_circle(image, cx + 3 * s, cy - 10 * s, 2.5 * s, spots)
	_fill_circle(image, cx + 6 * s, cy - 7 * s, 1.5 * s, spots)
	_fill_circle(image, cx - 7 * s, cy - 7 * s, 1.5 * s, spots)

	# Eyes
	_fill_circle(image, cx - 3 * s, cy - 3 * s, 2 * s, Color(0.1, 0.1, 0.1))
	_fill_circle(image, cx + 3 * s, cy - 3 * s, 2 * s, Color(0.1, 0.1, 0.1))
	_fill_circle(image, cx - 3 * s, cy - 3.5 * s, 1 * s, Color(0.3, 0.9, 0.2))
	_fill_circle(image, cx + 3 * s, cy - 3.5 * s, 1 * s, Color(0.3, 0.9, 0.2))

	return ImageTexture.create_from_image(image)

# ─── iron beetle sprite frames ───
...
static func _draw_beetle_frame(size: int, bob: float, leg_offset: float) -> Texture2D:
	...
	_fill_circle(image, size / 2.0, size / 2.0 + 10 * s, 9 * s, Color(0, 0, 0, 0.12))

	# Legs (6 legs)
	_fill_rect(image, cx - 10 * s, cy + 2 * s + leg_offset, 3 * s, 6 * s, dark)
	_fill_rect(image, cx + 7 * s, cy + 2 * s - leg_offset, 3 * s, 6 * s, dark)
	_fill_rect(image, cx - 12 * s, cy + 0 * s - leg_offset * 0.5, 3 * s, 5 * s, dark)
	_fill_rect(image, cx + 9 * s, cy + 0 * s + leg_offset * 0.5, 3 * s, 5 * s, dark)
	_fill_rect(image, cx - 8 * s, cy + 4 * s - leg_offset * 0.3, 2 * s, 5 * s, dark)
	_fill_rect(image, cx + 6 * s, cy + 4 * s + leg_offset * 0.3, 2 * s, 5 * s, dark)

	# Shell
	_fill_circle(image, cx, cy, 10 * s, shell)
	_fill_circle(image, cx, cy - 2 * s, 9 * s, light)
	_fill_rect(image, cx - 1 * s, cy - 8 * s, 2 * s, 14 * s, dark)
	_fill_circle(image, cx - 3 * s, cy - 4 * s, 3 * s, light.lightened(0.2))

	# Spikes
	_fill_rect(image, cx - 6 * s, cy - 8 * s, 2 * s, 3 * s, spike)
	_fill_rect(image, cx + 4 * s, cy - 8 * s, 2 * s, 3 * s, spike)
	_fill_rect(image, cx, cy - 10 * s, 2 * s, 3 * s, spike)

	# Head
	_fill_circle(image, cx, cy - 9 * s, 4 * s, dark)
	_fill_rect(image, cx - 4 * s, cy - 12 * s, 2 * s, 4 * s, spike)
	_fill_rect(image, cx + 2 * s, cy - 12 * s, 2 * s, 4 * s, spike)
	_fill_circle(image, cx - 2 * s, cy - 10 * s, 1.5 * s, Color(0.9, 0.5, 0.1))
	_fill_circle(image, cx + 2 * s, cy - 10 * s, 1.5 * s, Color(0.9, 0.5, 0.1))

	return ImageTexture.create_from_image(image)

# ─── banshee sprite frames ───
...
static func _draw_banshee_frame(size: int, float_y: float, sway: float, scream: float) -> Texture2D:
	...
	# Wispy bottom
	_fill_circle(image, cx, cy + 10 * s, 5 * s, wisp)
	_fill_circle(image, cx - 2 * s, cy + 13 * s, 3 * s, Color(wisp.r, wisp.g, wisp.b, 0.15))
	_fill_circle(image, cx + 3 * s, cy + 14 * s, 2 * s, Color(wisp.r, wisp.g, wisp.b, 0.1))

	# Body
	_fill_circle(image, cx, cy + 2 * s, 7 * s, body)
	_fill_circle(image, cx, cy - 2 * s, 6 * s, body)

	# Hair
	_fill_circle(image, cx, cy - 10 * s, 8 * s, hair)
	_fill_rect(image, cx - 8 * s, cy - 8 * s, 4 * s, 12 * s, hair)
	_fill_rect(image, cx + 4 * s, cy - 8 * s, 4 * s, 12 * s, hair)

	# Face
	_fill_circle(image, cx, cy - 7 * s, 5 * s, Color(0.9, 0.85, 0.9, 0.9))

	# Arms
	var arm_spread: float = 8 * s + scream * 4 * s
	_fill_rect(image, cx - arm_spread - 3 * s, cy - 2 * s, 4 * s, 2 * s, body)
	_fill_rect(image, cx + arm_spread - 1 * s, cy - 2 * s, 4 * s, 2 * s, body)

	# Eyes
	var eye_size: float = 2 * s + scream * 1 * s
	_fill_circle(image, cx - 3 * s, cy - 8 * s, eye_size, glow)
	_fill_circle(image, cx + 3 * s, cy - 8 * s, eye_size, glow)
	_fill_circle(image, cx - 3 * s, cy - 8 * s, eye_size * 0.5, Color(0.8, 1.0, 1.0, 0.8))
	_fill_circle(image, cx + 3 * s, cy - 8 * s, eye_size * 0.5, Color(0.8, 1.0, 1.0, 0.8))

	# Mouth (open during scream)
	if scream > 0.3:
		var mouth_size: float = 2 * s + scream * 2 * s
		_fill_circle(image, cx, cy - 4 * s, mouth_size, Color(0.1, 0.05, 0.15, 0.9))
		_fill_circle(image, cx, cy - 4 * s, mouth_size + 4 * s, Color(glow.r, glow.g, glow.b, 0.2))

	return ImageTexture.create_from_image(image)

# ─── bone colossus sprite frames ───
...
static func _draw_colossus_frame(size: int, bob: float, leg_offset: float) -> Texture2D:
	...
	_fill_circle(image, cx, cy + 14 * s, 11 * s, Color(0, 0, 0, 0.15))

	# Legs
	_fill_rect(image, cx - 7 * s, cy + 3 * s + leg_offset, 5 * s, 11 * s, bone)
	_fill_rect(image, cx + 2 * s, cy + 3 * s - leg_offset, 5 * s, 11 * s, bone)
	_fill_rect(image, cx - 6 * s, cy + 6 * s + leg_offset, 3 * s, 2 * s, dark)
	_fill_rect(image, cx + 3 * s, cy + 6 * s - leg_offset, 3 * s, 2 * s, dark)

	# Torso (fused ribcages)
	_fill_rect(image, cx - 9 * s, cy - 6 * s + bob, 18 * s, 10 * s, bone)
	_fill_rect(image, cx - 8 * s, cy - 4 * s + bob, 16 * s, 1 * s, dark)
	_fill_rect(image, cx - 7 * s, cy - 1 * s + bob, 14 * s, 1 * s, dark)
	_fill_rect(image, cx - 6 * s, cy + 2 * s + bob, 12 * s, 1 * s, dark)
	_fill_rect(image, cx - 1 * s, cy - 6 * s + bob, 2 * s, 10 * s, dark)
	_fill_circle(image, cx, cy - 2 * s + bob, 4 * s, glow)

	# Arms
	_fill_rect(image, cx - 13 * s, cy - 4 * s + bob, 5 * s, 12 * s, bone)
	_fill_rect(image, cx + 8 * s, cy - 4 * s + bob, 5 * s, 12 * s, bone)
	_fill_rect(image, cx - 11 * s, cy - 2 * s + bob, 3 * s, 6 * s, bone.darkened(0.15))
	_fill_rect(image, cx + 8 * s, cy - 2 * s + bob, 3 * s, 6 * s, bone.darkened(0.15))
	_fill_circle(image, cx - 11 * s, cy + 8 * s + bob, 3.5 * s, dark)
	_fill_circle(image, cx + 11 * s, cy + 8 * s + bob, 3.5 * s, dark)

	# Head (skull cluster)
	_fill_circle(image, cx, cy - 11 * s + bob, 6 * s, bone)
	_fill_circle(image, cx - 5 * s, cy - 9 * s + bob, 4 * s, bone.darkened(0.1))
	_fill_circle(image, cx + 5 * s, cy - 9 * s + bob, 4 * s, bone.darkened(0.1))
	_fill_circle(image, cx - 2 * s, cy - 12 * s + bob, 2 * s, socket)
	_fill_circle(image, cx + 2 * s, cy - 12 * s + bob, 2 * s, socket)
	_fill_circle(image, cx - 2 * s, cy - 12 * s + bob, 1 * s, glow)
	_fill_circle(image, cx + 2 * s, cy - 12 * s + bob, 1 * s, glow)
	_fill_circle(image, cx - 6 * s, cy - 10 * s + bob, 1 * s, glow)
	_fill_circle(image, cx + 6 * s, cy - 10 * s + bob, 1 * s, glow)
...
static func _draw_colossus_attack_frame(size: int, slam: float) -> Texture2D:
	...
	_fill_circle(image, cx, cy + 14 * s, 11 * s, Color(0, 0, 0, 0.15))
	_fill_rect(image, cx - 7 * s, cy + 3 * s, 5 * s, 11 * s, bone)
	_fill_rect(image, cx + 2 * s, cy + 3 * s, 5 * s, 11 * s, bone)
	_fill_rect(image, cx - 9 * s, cy - 6 * s + lean, 18 * s, 10 * s, bone)
	_fill_rect(image, cx - 1 * s, cy - 6 * s + lean, 2 * s, 10 * s, dark)
	_fill_circle(image, cx, cy - 2 * s + lean, 4 * s, Color(glow.r, glow.g, glow.b, 0.6 + slam * 0.3))
	var arm_raise: float = -8.0 * (1.0 - slam)
	_fill_rect(image, cx - 13 * s, cy - 4 * s + lean + arm_raise, 5 * s, 12 * s, bone)
	_fill_rect(image, cx + 8 * s, cy - 4 * s + lean + arm_raise, 5 * s, 12 * s, bone)
	_fill_circle(image, cx - 11 * s, cy + 8 * s + lean + arm_raise, 3.5 * s, dark)
	_fill_circle(image, cx + 11 * s, cy + 8 * s + lean + arm_raise, 3.5 * s, dark)
	_fill_circle(image, cx, cy - 11 * s + lean, 6 * s, bone)
	_fill_circle(image, cx - 5 * s, cy - 9 * s + lean, 4 * s, bone.darkened(0.1))
	_fill_circle(image, cx + 5 * s, cy - 9 * s + lean, 4 * s, bone.darkened(0.1))
	var eye_bright: float = 0.4 + slam * 0.5
	_fill_circle(image, cx - 2 * s, cy - 12 * s + lean, 1.5 * s, Color(eye_bright, 1.0, eye_bright * 0.8, 0.9))
	_fill_circle(image, cx + 2 * s, cy - 12 * s + lean, 1.5 * s, Color(eye_bright, 1.0, eye_bright * 0.8, 0.9))
